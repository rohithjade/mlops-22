...
from utils import get_all_h_param_comb_svm, tune_and_save, train_dev_test_split,preprocess_digits
...
def test_data_set_split_same():
    x_train1, y_train1, x_dev1, y_dev1, x_test1, y_test1 = train_dev_test_split(data, label, train_frac, dev_frac,rd=1)
    x_train2, y_train2, x_dev2, y_dev2, x_test2, y_test2 = train_dev_test_split(data, label, train_frac, dev_frac,rd=1)

    flag = True

    if not np.array_equal(x_train1,x_train2):
        flag = False
    
    if not np.array_equal(y_train1,y_train2):
        flag = False
    
    if not np.array_equal(x_dev1,x_dev2):
        flag = False

    if not np.array_equal(y_dev1,y_dev2):
        flag = False

    if not np.array_equal(x_test1,x_test2):
        flag = False
...
def test_data_set_split_notsame():
    x_train1, y_train1, x_dev1, y_dev1, x_test1, y_test1 = train_dev_test_split(data, label, train_frac, dev_frac,rd=1)
    x_train2, y_train2, x_dev2, y_dev2, x_test2, y_test2 = train_dev_test_split(data, label, train_frac, dev_frac,rd=2)
